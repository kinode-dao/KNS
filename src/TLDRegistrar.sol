...
contract TLDRegistrar is ITLDRegistrar {
    ...
    function __TLDRegistrar_init(
        ...
        kns = IKNSRegistryResolver(_kns);

        _name = _name;
        _symbol = _symbol;
    }

    // called by KNSRegistry as safety for the correct TLD
    function __initTLDRegistration(
        bytes calldata _fqdn,
        bytes32 _tldHash
    ) public {
        if (TLD_HASH != bytes32(0)) revert AlreadyRegistered();
        if (msg.sender != address(kns)) revert KNSRegistryOnly();
        TLD_HASH = _tldHash;
        TLD_DNS_WIRE = _fqdn;
    ...
    function _setNode(bytes32 _node, uint256 _key) internal returns (bytes32) {
    ...
    function _getAttributes(bytes32 node) internal pure returns (bytes32) {
        return node & MASK_RIGHT_96;
    }

    function _getOwner(bytes32 node) internal pure returns (address) {
        return address(uint160(uint256(node) >> 96));
    }

    function _ownerOf(uint _node) internal view returns (address) {
    ...
    function _register(
    ...
    ) internal returns (uint256 nodeId_) {
        bytes32 _node = kns.registerNode(_name);
        ...
        if (0 < _data.length) kns.multicallWithNodeCheck(_node, _data);
    }

    function auth(
        bytes32 _node,
        address _sender
    ) public view virtual returns (bool authed_) {
    ...
    function _getNodeAndParent(
        bytes memory fqdn
    ) internal pure returns (bytes32 node, bytes32 parentNode) {
