...
import { fillAndSign, getUserOpHash, UserOperation } from './UserOperation'
import {
  EntryPoint,
  EntryPoint__factory,
  SimpleAccount,
...
export class AASigner extends Signer {
  ...
  async syncAccount (): Promise<void> {
    ...
    if (this._isPhantom) {
      ...
      // .log(`== __isPhantom. addr=${this._account.address} re-checking code size. result = `, size)
  ...
  async _createUserOperation (
    ...
    const userOp = await fillAndSign({
      ...
      initCode,
      nonce: initCode == null ? tx.nonce : 0,
