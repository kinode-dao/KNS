...
import "../src/DotOsRegistrar.sol";
import "../src/KNSRegistryResolver.sol";
...
contract TLDShim is TLDRegistrar {
    ...
    function init(
        ...
        __TLDRegistrar_init(_kns, _name, _symbol);
    }
...
contract DotOsShim is DotOsRegistrar {
...
contract User {
    TLDShim public tld;
    DotOsShim public dotuq;
    KNSRegistryResolver public kns;

    constructor(address _kns, address _dotuq, address _tld) {
        kns = KNSRegistryResolver(_kns);
        dotuq = DotOsShim(_dotuq);
        tld = TLDShim(_tld);
    }

    function setKey(bytes32 _node, bytes32 _key) public {
        kns.setKey(_node, _key);
    }
}
