...
contract LZEndpointMock is ILayerZeroEndpoint {
    ...
    bool public nextMsgQueued = true;
    ...
    mapping(uint16 => mapping(bytes => uint64)) public outboundNonceUA;
    ...
    function send(uint16 _chainId, bytes memory _path, bytes calldata _payload, address payable _refundAddress, address _zroPaymentAddress, bytes memory _adapterParams) external payable override {
        ...
        ++outboundNonce[_chainId][msg.sender];
        uint64 nonce = ++outboundNonceUA[_chainId][_path];
        ...
        (, uint extraGas, uint dstNativeAmt, address payable dstNativeAddr) = decodeAdapterParams(adapterParams);
        if (dstNativeAmt > 0) {
    ...
    function send(uint16 _cid, bytes memory _path, address payable refundaddr) external payable {
    ...
    function receivePayload(uint16 _srcChainId, bytes calldata _path, address _dstAddress, uint64 _nonce, uint _gasLimit, bytes calldata _payload) external override {
        ...
        if (!nextMsgQueued) require(_nonce == ++inboundNonce[_srcChainId][_path], "LayerZeroMock: wrong nonce");

        // queue the following msgs inside of a stack to simulate a successful send on src, but not fully delivered on dst
        if (sp.payloadHash != bytes32(0) || nextMsgQueued) {
            QueuedPayload[] storage msgs = msgsToDeliver[_srcChainId][_path];
            QueuedPayload memory newMsg = QueuedPayload(_dstAddress, _gasLimit, _nonce, _payload);
    ...
    function executeQueuedMsg (uint16 _srcChainId, bytes calldata _path) public {

      QueuedPayload[] storage msgs = msgsToDeliver[_srcChainId][_path];

      require(msgs.length != 0, "lz mock: no messages");

      QueuedPayload memory qdmsg = msgs[msgs.length-1];

      require(qdmsg.nonce == ++inboundNonce[_srcChainId][_path], "LayerZeroMock: wrong nonce");

      msgs.pop();

      try ILayerZeroReceiver(qdmsg.dstAddress).lzReceive{gas: qdmsg.gaslimit}(_srcChainId, _path, qdmsg.nonce, qdmsg.payload) {} catch (bytes memory reason) {
        storedPayload[_srcChainId][_path] = StoredPayload(uint64(qdmsg.payload.length), qdmsg.dstAddress, keccak256(qdmsg.payload));
        emit PayloadStored(_srcChainId, _path, qdmsg.dstAddress, qdmsg.nonce, qdmsg.payload, reason);
      }
    ...
    function getOutboundNonceUA(uint16 _chainID, bytes calldata _path) external view returns (uint64) {
        return outboundNonceUA[_chainID][_path];
    }
    ...
    function queueNextMsg() external {
        nextMsgQueued = true;
    }
